...
export function AdvancedWorkflowDemo() {
  ...
  // ── Mobile canvas scale: shrink the 500×460 fixed layout to fit any screen width ──
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const update = () => setNodeScale(Math.min(1, el.offsetWidth / DESIGN_W));
    update();
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        ...
        <div className="relative mb-20 md:mb-32">
          ...
          >
            ...
            >
              {/* Canvas stage — measures available width to compute scale */}
              <div className="relative flex flex-col items-center px-4 md:px-8 lg:px-16 py-8 md:py-14">
                {/* Invisible sizer: tracks available width for ResizeObserver */}
                <div ref={canvasWrapRef} className="w-full" style={{ maxWidth: DESIGN_W }} />

                {/* Outer shell that collapses to the scaled height */}
                <div style={{ position: 'relative', width: DESIGN_W * nodeScale, height: DESIGN_H * nodeScale, flexShrink: 0 }}>
                  {/* Fixed design canvas, scaled from top-left */}
                  <div
                    style={{
                      position: 'absolute',
                      top: 0,
                  ...
                  >
                    <AnimatedConnection id="c1" isActive={activeStep >= 1}
                      from={{ x: 170, y: 60 }} to={{ x: 170, y: 200 }} />
                    <AnimatedConnection id="c2" isActive={activeStep >= 2}
                      from={{ x: 170, y: 200 }} to={{ x: 30, y: 360 }} />
                    <AnimatedConnection id="c3" isActive={activeStep >= 2}
                      from={{ x: 170, y: 200 }} to={{ x: 310, y: 360 }} />
                    {WORKFLOW_NODES.map((node, index) => (
                      <WorkflowNode key={node.id} node={node} isActive={activeStep >= index} />
                    ))}
                  </div>
                </div>
              </div>
